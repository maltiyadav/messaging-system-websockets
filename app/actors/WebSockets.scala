...
import actors.WebSockets.Message
import actors.WebSockets.Message.messageReads
...
object WebSockets {
  def props(user: String)(out: ActorRef) = Props(new WebSockets(user, out))
  def topicMsgKey(topic: String) = LWWRegisterKey[UsersMessage](topic + "-lwwreg")
  ...
  case class ChatMessagesListMessage(msgs: Seq[UsersMessage])
...
class WebSockets(uid: String, out: ActorRef) extends Actor with ActorLogging {
  import WebSockets._

  val topicsKey = GSetKey[String]("topics")
  var lastSubscribed: Option[String] = None
  var initialHistory: Option[Set[UsersMessage]] = None
  ...
  def afterTopics = LoggingReceive {
    ...
    case c @ Changed(LWWRegisterKey(topic)) =>
      for {
        ...
        val chatMessage = c.get(LWWRegisterKey[UsersMessage](topic)).value
    ...
    case g @ GetSuccess(GSetKey(topic), req) =>
      for {
        ...
        val elements = g.get(GSetKey[UsersMessage](topic)).elements
    ...
    case g @ NotFound(GSetKey(topic), req) =>
      for {
        ...
        initialHistory = Some(Set.empty[UsersMessage])
        replicator ! Subscribe(topicMsgKey(topic), self)
      }
        ...
        case "message" =>
          js.validate[Message](messageReads)
            ...
            .foreach { case (topic, msg) => 
              val chatMessage = UsersMessage(topic, uid, msg, new java.util.Date())
              replicator ! Update(GSetKey[UsersMessage](topic), GSet.empty[UsersMessage], WriteLocal) {
                _ + chatMessage
              }
              replicator ! Update(topicMsgKey(topic), LWWRegister[UsersMessage](null), WriteLocal) {
